...
def fetch(url):
    ...
    with urllib.request.urlopen(req,timeout=30) as r: return r.read(),r.status,r.url,r.headers.get('content-type','')
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--lender'); ap.add_argument('--verbose',action='store_true'); args=ap.parse_args()
    ...
    for lender in cfg:
        if not lender.get('enabled',True) or (args.lender and lender['id']!=args.lender): continue
        ...
        try:
            ...
            rec.update({'category':lender['category'],'source_url':final,'source_type':lender.get('source_type','HTML').upper(),'last_verified':now[:10],'collection_timestamp':now})
