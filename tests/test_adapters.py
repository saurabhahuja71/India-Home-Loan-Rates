...
class AdapterTests(unittest.TestCase):
    def test_explicit_home_table_selects_home_loan_not_lap(self):
        ...
        self.assertEqual(result['rate_min'],8.50)
        self.assertIn('Home Loan',result['source_row'])
        self.assertNotIn('Mortgage',result['source_row'])
        self.assertEqual(result['rate_source_column'],'Interest Rate')
